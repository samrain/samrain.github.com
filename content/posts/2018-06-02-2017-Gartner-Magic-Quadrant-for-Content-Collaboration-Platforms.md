...
Box的服务仅在公共云中可用, 在美国的数据中心实现, 或者通过 AWS、IBM 和 Microsoft (不久可获得) 其他区域的本地数据中心。Box支持用户的生产力和协作跨任何设备-集成与微软 Office 365 和谷歌 G 套件。还提供了增强的内容流、实时编辑、机器学习和分析功能。框提供了丰富的安全性、法规遵从性和管理功能, 如云数据保护和治理功能, 包括存储在云中的客户内容的加密密钥管理 (Box KeySafe) 和灵活的内容地理位置用于存储的技术 (Box Zones)。
...
- Google 对企业优先级和需求 (包括 it 用户、it 管理员和企业主) 有了新的方法。在过去的15月里, 一个新的执行团队和老鸟们加入了谷歌, 为产品管理带来了更多的人和高级主管。组织的变化发生在整个公司, 包括谷歌的驱动团队。引用表示更好的客户支持和路线图可见性。
...
Intralinks 的 Intralinks 是通过与面向外部的业务流程和敏感数据的企业安全内容共享和协作而实现的。Intralinks 还传统上提供了 Intralinks 的虚拟数据室版本, 用于专门的用例, 如虚拟数据室、贷款联合、合并和收购 (M&A) 交易、私募股权融资和临床试验。随着收购, Intralinks 产品进入市场两种不同的服务: Synchronoss 企业销售给所有企业;和 Intralinks 为战略金融, 一个 VDR 提供支持敏感的 M&A 和交易活动。
...
- AeroFS: 它提供了一个由对等技术和一个中央虚拟设备管理的内部的CCP。互补产品 Amium 支持内容协作。缺少标准: 提供CCP云, 收入。
-     亚马逊: 它为内容协作提供了一个完整的产品, 亚马逊 WorkDocs 作为 AWS 的一部分, 面向 MSE 市场。缺少标准: 部署大小。
-     Autotask: 收购了CCP云供应商 Soonr 后, 它于2017年5月推出了一个CCP产品。缺少标准: 可用性、收入和部署大小。
-     CodeLathe: 它提供了私有云文件共享工具, 用户可以访问、同步和共享来自任何设备 Tonido 的文件, 用于企业的消费者和 FileCloud。缺少标准: 收入。
-     Egress Software Technologies:: 它使文件共享和协作与电子邮件加密, 文件传输和安全协作。这是通过其旗舰的CCP解决方案 , 出换机交付。缺少标准: 收入。
-  Eisoo: 这家中国供应商提供的 AnyShare 产品在亚太地区同步和共享。缺少标准: 地理覆盖。
-  ibm: 它在云上提供 ibm 连接文件, 这是连接云家族的一个不可分割的组成部分。作为 IBM 连接的扩展, 它不符合包含。缺少标准: 独立包。
- ownCloud: 它提供开放源码的 CCP 软件, 可在免费社区版和付费订阅的企业版中获得。这是一个目标产品, 可以部署在前提下或托管在第三方云。缺少标准: 收入。
- Vaultize: 它为CCP提供了移动内容管理和 DLP 安全访问企业数据的机会。缺少标准: 收入和付费用户总数。
- Workshare: 它提供了一个CCP的协作平台, 可在公共云或混合产品中使用。Workshare 在监管部门有很大的存在。缺少标准: 收入。

在各种市场中, 其他供应商在其产品中提供了CCP的特色。他们错过了一个独立的 CPP 产品;因此, 他们不符合纳入这个魔术象限。示例有:

- 业务应用: Salesforce, SAP
-     协作: CommandHub, Hightail, Micro Focus (Novell), SpiderOak, SpringCM, Zoho
-     备份: Acronis, Druva, Commvault, Intermedia
-     门户: Brainloop, Boardvantage, Diligent
...
-     项目管理: Redbooth Active Collab 
-     存储: Barracuda Networks, Hitachi Data Systems (HDS) 
-     虚拟数据室 (VDR): ansarada, CapLinked, Firmex, Merrill, Onehub 

## 纳入和排除标准
为了符合此魔术象限中的包含条件, 供应商必须满足以下标准:

- 提供: 它有一个CCP，提供核心 EFSS 能力。
-     收入: 2016 的CCP和服务收入必须2000万美元以上。
-     地理: 它至少有两个地理区域存在, 有些人员专门负责相关产品。不超过70% 的收入可能来自一个地理区域。
-     商业可用性: 自2016年9月1日以来, CPP 产品普遍可用。
-     包装: CCP的产品必须作为一个单独计费, 独立的产品。仅通过与同一供应商的不同产品捆绑在一起的 CCP 功能被视为扩展, 并且不足以包含在内。
-     总用户: 在许可使用该产品的所有组织中, 必须至少有50万个活动的、付费的用户。
-     最大部署: 至少有一个部署必须有1万个用户。
-     参考: 五客户必须已经部署了至少六月的服务或产品, 并拥有最少1000付费用户。其中的两个必须至少有4000付费用户。

此外, 他们的CCP必须支持选定的产品功能范围:

- 文件同步: 这是支持在设备和云服务/服务器之间, 或跨多个设备之间的透明和自动往返数据同步, 用于选定的文件或文件夹。
-     文件共享: 这是对属于同一个人的设备之间的多层次文件 (和文件夹) 共享的支持;设备上的不同应用;以及组织内部和/或外部的人员。
-     文件访问: 这是从客户端应用程序或浏览器到存储在内部部署或云存储库中的文件的访问, 通过直接访问或复制到内存储库。使用第三方连接器是可以接受的, 但本地支持的额定较高。
-     用户生产力: 这包括移动、桌面和浏览器应用程序中的文档视图、编辑和批注, 无论是通过嵌入式本地功能还是通过第三方工具 (例如, Office 365)。
-     手机: 这包括一个本机应用程序的 iOS 和 Android。对 Windows 和其他平台的支持是可选的。
-     个人电脑: 这是支持在 Windows pc 和 Mac OS 通过一个本地独立的应用程序的同步。Web 浏览器或电子邮件客户端插件以及对 Linux 平台的支持都是可选的。
-     安全性: 这包括密码身份验证, 在一段时间的不活动后锁定, 在设备上选择远程擦除应用程序和相关文件, 以及在 rest 和传输时进行数据加密。
-     管理: 这是指与 LDAP 和活动目录的集成, 用于身份验证、单一登录、组策略和集中式管理工具, 允许管理员管理同步并控制内容、访问权限和用户活动。与 EMM 集成是可选的。
-     集成: 产品必须支持至少一种类型的集成与企业数据存储库, 在内部或云-例如, 微软 SharePoint, 微软 OneDrive 业务, Google Drive, Dropbox, AWS 和 Salesforce-或数据基础设施如网络连接存储。
-     平台: 例如, 产品必须包含一个内容平台, 其中包括 api、导航、文档、开发工具、应用程序管理和合作伙伴的生态系统。
-     交付模式: 这意味着在公共或私有云中提供云服务, 或者作为混合部署, 将内部存储库与基于云的 CCP 功能结合在一起。

合作伙伴提供的功能必须与供应商的产品紧密集成, 最终用户不可见。
...
在这个神奇的象限中, 一看供应商, 就可以识别出不同的宏类别供应商 (虽然, 有些提供多个类别的支持):
...
**业务**: 本组织实现其目标和承诺的能力。因素包括组织结构的质量, 包括技能、经验、方案、系统和其他车辆, 使本组织能够有效和有效地持续运作。
